...
@testset "function eval" begin
    @test size(SurrogateModelOptim.f_opt_eval(minimum,plan,false)) == (1,n_samples)
    samples = mapslices(minimum,plan,dims=1)
    @test size(SurrogateModelOptim.f_opt_eval(minimum,plan,samples,false)) == (1,n_samples)
end